export default function mask(value, precision = 2, decimalSeparator = '.', thousandSeparator = ',', allowNegative = false, prefix = '', suffix = '', autoAdd = true) {
    ...
    if (value.indexOf(decimalSeparator) !== -1) {
        let decString = value.substr(value.indexOf(decimalSeparator) + 1);
        declen = decString.length;
        if (decString.length < precision) {
            for (let preLen = 0; preLen < (precision - decString.length); preLen++) {
                digits.push('0');
            }
        }
    } else {
        for (let preLen = 0; preLen < precision; preLen++) {
            digits.push('0');
        }
    }
    ...
    precision = autoAdd ? precision : declen;
